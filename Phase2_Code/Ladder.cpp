...
bool Ladder::IsOverlapping(GameObject* newObj) const
{
	...
	if (pr != NULL)
	{
		...
		int vcellnewend = pr->GetEndPosition().VCell();
		int hcellnewend = pr->GetEndPosition().HCell();
		if (hcellnew == hcellcurr)
		{
			if (vcellcurr > vcellnew && vcellcurrend <= vcellnew)
			{
				return true;
			}
			if (vcellnew > vcellcurr && vcellnewend <= vcellcurr)
			{
				return true;
			}
			if (vcellnew == vcellcurr || vcellnewend == vcellcurrend)
			{
				return true;
			}
		}
